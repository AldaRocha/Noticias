...
function fecha(){
    ...
    let fechaActualCadena=`${dia < 10 ? '0' + dia : dia}/${mes < 10 ? '0' + mes : mes}/${anio}`;
    return fechaActualCadena;
}

function accesar(){
    let nombreUsuario=document.getElementById("idUsuario").value;
    let contrasenia=document.getElementById("idPassword").value;
                            ...
                            fetch("api/respuesta/getAll")
                                    .then(response=>{
                                        return response.json();
                                            ...
                                            .then(dataR=>{
                                                if(dataR.error){
                                                    alert("Error al solicitar los datos de respuestas a la base de datos");
                                                } else{
                                                    respuestas=dataR;
                                                    ...
                                                    noticias.forEach(function(noticia){
                                                        let comentariosNoticia=comentarios.filter(function(comentario){
                                                    ...
                                                    comentariosNoticia.forEach(function(comentario){
                                                        registroComentarios+=`<h4>`+comentario.texto+`</h4>
                                                        <input type="text" id="txtR`+comentario.idComentario+`" class="mb-2" placeholder="Responder a este comentario"/>
                                                        <button class="btn btn-primary mb-2" onclick="enviarRespuesta(`+comentario.idComentario+`);">Responder</button>
                                                        <hr>`;
                                
                                                        let respuestasFiltradas=respuestas.filter(function(respuesta){
                                                            return respuesta.comentario.idComentario===comentario.idComentario;
                                                        });
                                                        
                                                        respuestasFiltradas.forEach(function(respuesta){
                                                            registroComentarios+=`<h5>`+respuesta.texto+`</h5>
                                                                                  <hr>
                                                                                  <hr>`;
                                                        });
                                                    ...
                                                    let registro=`<div class="card col-lg-4 align-content-center d-flex">
                                          <h6 class="text-end">`+noticia.fechaPublicacion.substring(0, 10)+`</h6>
                                          <h1 class="text-center">`+noticia.titulo+`</h1>
                                          <hr>
                                          <h2 class="text-center">`+noticia.contenido+`</h2>
                                          <hr>
                                          <h3 class="text-end">Escrito por `+noticia.personal.nombre+`</h3>
                                          <hr>
                                          <h4>Comentarios</h4>
                                          `+registroComentarios+`
                                          <input type="text" id="txtC`+noticia.idNoticia+`" class="mb-2" placeholder="Escribe tu comentario"/>
                                          <button class="btn btn-success mb-2" onclick="enviarComentario(`+noticia.idNoticia+`);">Enviar</button>
                                          </div>`;                    
                        cuerpo += registro;
                    });
